...
/**
 * Base implementation for Mappers that requires the implementation of.
 * * convertToDTO
 * * convertToEntity
 * * convertQuery
 *
 */
export abstract class AbstractMapper<DTO, Entity, C = DeepPartial<DTO>, CE = DeepPartial<Entity>, U = C, UE = CE>
  implements Mapper<DTO, Entity, C, CE, U, UE>
{
  ...
  /**
   * @param DTOClass - Optional class definition for the DTO. If not provided it will be looked up from the \@Mapper annotation.
   * @param EntityClass - Optional class definition for the entity. If not provided it will be looked up from the \@Mapper annotation.
   */
  constructor(DTOClass?: Class<DTO>, EntityClass?: Class<Entity>) {
    const classes = getMapperClasses(this.constructor as Class<Mapper<DTO, Entity, C, CE, U, UE>>);
    ...
    if (!DTOClas || !EntityClas) {
      ...
      throw new Error(
        `Unable to determine DTO or Entity types for ${this.constructor.name}. Did you annotate your mapper with @Mapper`,
      );
    }
